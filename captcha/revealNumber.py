...
def imageToNumber(img, invert=False):
    global saved_image_index
    cv2.imwrite('./{}-img.png'.format(saved_image_index), img)
    ...
    cv2.imwrite('./{}-bw.png'.format(saved_image_index), img)
    saved_image_index += 1

    text = pytesseract.image_to_string(
        img, config='--psm 8 -c tessedit_char_whitelist={}'.format(whitelist))
...
def solveCaptcha():
    ...
    # target = imageToNumber(img, invert=True)
    ...
    # if len(target) != 3:
    #     print('cannot convert image to number!')
